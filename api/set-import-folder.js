...
async function resolveParentFolder(drive, folderName, existingFolderId, parentFolderId) {
    if (parentFolderId) {
        const verified = await verifyFolderAccess(drive, parentFolderId);
        if (verified) return { parentFolderId: verified.id, parentFolderName: verified.name };
        return { accessLost: true };
    }

    if (!folderName || typeof folderName !== 'string' || !folderName.trim()) {
        return { parentFolderId: null, parentFolderName: null };
    }
    ...
    if (existingFolderId) {
        const chosenFolder = existingFolders.find((f) => f.id === existingFolderId) || existingFolders[0];
        if (chosenFolder) {
            return { parentFolderId: chosenFolder.id, parentFolderName: chosenFolder.name };
        }
    }

    if (!existingFolders.length) {
        const folder = await createDriveFolder(drive, trimmedFolderName);
        return { parentFolderId: folder.id, parentFolderName: folder.name };
    }
...
module.exports = async function handler(req, res) {
    ...
    const { folderName, existingFolderId, parentFolderId } = req.body || {};
    ...
    try {
        ...
        oauth2Client.on('tokens', async (newTokens) => {
            try {
                const updateData = { encryptedAccessToken: encrypt(newTokens.access_token), updatedAt: new Date() };
        ...
        const folderResult = await resolveParentFolder(drive, folderName, existingFolderId, parentFolderId);

        if (folderResult.accessLost) {
            return res.status(410).json({ error: 'folder_access_lost', message: 'The previously used folder is no longer accessible.' });
        }

        const isV2 = req.body.version === 2;
        const updateFields = { updatedAt: new Date() };
        if (folderResult.parentFolderId) {
            updateFields[isV2 ? 'importFolderIdV2' : 'importFolderId'] = folderResult.parentFolderId;
            updateFields[isV2 ? 'importFolderNameV2' : 'importFolderName'] = folderResult.parentFolderName;
        } else {
            updateFields[isV2 ? 'importFolderIdV2' : 'importFolderId'] = null;
            updateFields[isV2 ? 'importFolderNameV2' : 'importFolderName'] = null;
        }
        ...
        await AuditLog.create({
            ...
            action: 'set_import_folder',
            fileId: folderResult.parentFolderId,
            ip: parseIp(req),
            status: 'success',
        });
        ...
        console.error('Set import folder error:', err);
        const statusCode = err.statusCode || 500;
        res.status(statusCode).json({ error: err.message, detail: err.message });
    }
};
